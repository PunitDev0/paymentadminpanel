...
const MunicipalityDashboard = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      console.log('Fetched Municipality Data:', result);
  ...
  const operators = filteredData.reduce((acc, item) => {
    const operator = item.operatorname || 'Unknown';
    if (!acc[operator]) {
  ...
  const exportData = () => {
    ...
    const csv = [
      ...
      ...filteredData.map(item => [
        escapeCSV(item.txnid || ''),
        escapeCSV(item.operatorname || ''),
        escapeCSV(item.canumber || ''),
        item.amount || 0,
        item.comm || 0,
        item.tds || 0,
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <h1 className="text-2xl font-bold text-gray-800">Municipality Dashboard</h1>
                    ...
                    <div key={index} className="flex items-center justify-between">
                      <div className="flex items-center">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">Comm: ₹{operator.comm.toLocaleString()}</p>
                        </div>
                      </div>
                ...
                <table className="w-full border-collapse text-sm">
                  <thead>
                    <tr className="bg-gray-100 text-left text-gray-600 font-medium">
                      ...
                      <th className="p-3 border-b">CA Number</th>
                    ...
                    {filteredData.length > 0 ? (
                      filteredData.map((item, index) => (
                        <tr key={index} className="hover:bg-gray-50">
                          <td className="p-3 border-b">{item.txnid || 'N/A'}</td>
                          <td className="p-3 border-b">{item.operatorname || 'N/A'}</td>
                          <td className="p-3 border-b">{item.canumber || 'N/A'}</td>
                          <td className="p-3 border-b">₹{parseFloat(item.amount || 0).toLocaleString()}</td>
                          <td className="p-3 border-b">₹{parseFloat(item.comm || 0).toLocaleString()}</td>
                          <td className="p-3 border-b">₹{parseFloat(item.tds || 0).toLocaleString()}</td>
                          <td className="p-3 border-b">{item.status || 'N/A'}</td>
